package fr.jason.proxyhypixel.gui;
...
public class ProxyManagerGUI extends GuiScreen {
    ...
    public void initGui() {
        ...
        enableButton = new GuiButton(0, width / 2 - 100, height / 2 - 100, 200, 20, "Enable proxy: " + (ProxyHypixelConfig.proxyEnabled ? "§a" : "§c") + ProxyHypixelConfig.proxyEnabled);

        typeButton = new GuiButton(1, width / 2 - 100, height / 2 - 75, 200, 20, "Type: " + ProxyHypixelConfig.proxyType);

        hostField = new GuiTextField(2, fontRendererObj, width / 2 - 100, height / 2 - 50, 200, 20);
        hostField.setMaxStringLength(100);
        hostField.setText(ProxyHypixelConfig.proxyAddress);

        usernameField = new GuiTextField(3, fontRendererObj, width / 2 - 100, height / 2, 200, 20);
        usernameField.setMaxStringLength(100);
        usernameField.setText(ProxyHypixelConfig.proxyUsername);

        passwordField = new GuiTextField(4, fontRendererObj, width / 2 - 100, height / 2 + 25, 200, 20);
        passwordField.setMaxStringLength(100);
        passwordField.setText(ProxyHypixelConfig.proxyPassword);
    ...
    protected void actionPerformed(GuiButton button) {
        if (button.id == 0) {
            ProxyHypixelConfig.proxyEnabled = !ProxyHypixelConfig.proxyEnabled;
            enableButton.displayString = "Activer le proxy: " + (ProxyHypixelConfig.proxyEnabled ? "§a" : "§c") + ProxyHypixelConfig.proxyEnabled;
        } else if (button.id == 1) {
            if (ProxyHypixelConfig.proxyType == Proxy.ProxyType.SOCKS) {
                ProxyHypixelConfig.proxyType = Proxy.ProxyType.HTTP;
            } else {
                ProxyHypixelConfig.proxyType = Proxy.ProxyType.SOCKS;
            }
            typeButton.displayString = "Type: " + ProxyHypixelConfig.proxyType;
        } else if (button.id == 5) {
            Proxy.getInstance().setProxy(ProxyHypixelConfig.proxyEnabled, hostField.getText(), ProxyHypixelConfig.proxyType, usernameField.getText(), passwordField.getText());
