package fr.jason.proxyhypixel.transformer;
...
public class NetworkManagerTransformer implements IClassTransformer {
    ...
    public byte[] transform(String name, String transformedName, byte[] basicClass) {
        if (!name.equals("fr.jason.proxyhypixel.mixin.network.MixinNetworkManager")) {
            return basicClass;
        }
        try {
            ...
            classNode.methods.stream().filter(methodNode -> methodNode.name.equals("createNetworkManagerAndConnect")).forEach(methodNode -> {
                for (int i = 0; i < methodNode.instructions.size(); ++i) {
                    ...
                    if (abstractInsnNode instanceof TypeInsnNode) {
                        ...
                        if (tin.desc.equals("fr/jason/proxyhypixel/mixin/network/MixinNetworkManager$1")) {
                        ...
                        if (min.owner.equals("fr/jason/proxyhypixel/mixin/network/MixinNetworkManager$1") && min.name.equals("<init>")) {
